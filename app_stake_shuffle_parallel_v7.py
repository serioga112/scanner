...
def match_debug():
    try:
        ...
        stake_1x2 = _dedupe_1x2(combined, "Stake")
        shuffle_1x2 = _dedupe_1x2(combined, "Shuffle")
        matches = find_cross_book_matches(combined)
        return jsonify({
            "ok": True,
            "fetchSeconds": fetch_seconds,
            "stakeMarketsLoaded": len(stake_data.get("markets", [])),
            "shuffleMarketsLoaded": len(shuffle_data.get("markets", [])),
            "stake1x2Events": len(stake_1x2),
            "shuffle1x2Events": len(shuffle_1x2),
