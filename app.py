...
SCANNER_VERSION = "v8-multimarket"
...
# ---------------------------------------------------------------------------
# v8 multi-market arbitrage engine
# ---------------------------------------------------------------------------
# v8 deliberately limits totals/handicaps to half-goal lines (x.5). Quarter
# Asian lines and whole-number push lines require settlement-aware math and are
# therefore excluded from the "guaranteed profit" calculation for safety.
V8_SUPPORTED_FAMILIES = ("1x2", "moneyline", "total", "spread")
...
def _is_main_match_market(market):
    """Reject props/period markets whose settlement rules differ from full match."""
...
def _line_is_half(value):
    """Only .5 lines have no push/quarter-line settlement branch."""
    if value is None:
        return False
...
def _selection_team_side(market, outcome):
    """Return 1/2 for a team selection, tolerating handicap text around the name."""
    import re
    ...
    # Remove common handicap/odds annotations before team-name comparison.
...
def _extract_total_line(market):
    # Prefer structured provider line metadata.
    structured = _coerce_float(market.get("lineValue"))
    if structured is not None:
        return structured

    # Selection labels often carry "Over 2.5" / "Under 2.5".
    lines = []
    for outcome in market.get("outcomes", []) or []:
        if _over_under_side(outcome):
...
def _extract_home_spread_line(market):
    # Prefer explicit lines embedded in each team's selection label because they
    # establish which sign belongs to home/team1.
    home_lines = []
    away_lines = []
    for outcome in market.get("outcomes", []) or []:
        side = _selection_team_side(market, outcome)
        nums = _numbers_from_text(_selection_raw(outcome))
        if not nums:
            continue
        line = nums[-1]
...
def _market_descriptor_v8(market, swapped=False):
    """Return a canonical comparable market descriptor or None."""
    if not _is_main_match_market(market):
        return None
    ...
    if len(outcomes) == 3 and _canonical_market_type(market) == "1x2":
        return {"family": "1x2", "key": "1x2", "label": "1X2"}

    if len(outcomes) != 2:
        return None

    ou_sides = {_over_under_side(x) for x in outcomes}
    ou_sides.discard(None)
    looks_total = (
        ou_sides == {"over", "under"}
        or _contains_any(raw, (" total ", "over under", "over and under", "number of goals"))
    )
    if looks_total and ou_sides == {"over", "under"}:
        ...
        return {"family": "total", "key": key, "line": line, "label": f"Total {_line_key(line)}"}

    looks_spread = _contains_any(" " + raw + " ", (
        " handicap ", " spread ", " asian handicap ", " match handicap "
    ))
    if looks_spread:
        sides = {_selection_team_side(market, x) for x in outcomes}
        sides.discard(None)
        if sides != {"1", "2"}:
            return None
        home_line = _extract_home_spread_line(market)
        ...
        return {"family": "spread", "key": key, "line": home_line, "label": f"Handicap {_line_key(home_line)}"}

    # Two-way winner/moneyline. Exclude draw-no-bet because a draw refund needs
    # different settlement math and therefore is not a guaranteed two-way arb.
    if _contains_any(raw, ("draw no bet", " dnb ")):
        return None
    looks_moneyline = _contains_any(" " + raw + " ", (
        " winner ", " moneyline ", " money line ", " twoway ", " two way ", " match winner "
    ))
    if looks_moneyline:
        sides = {_selection_team_side(market, x) for x in outcomes}
        sides.discard(None)
        if sides == {"1", "2"}:
            return {"family": "moneyline", "key": "moneyline", "label": "2-Way Moneyline"}

    return None


def _canonical_selection_v8(market, outcome, descriptor, swapped=False):
    ...
    if family == "1x2":
        ...
        return label
    if family in {"moneyline", "spread"}:
        label = _selection_team_side(market, outcome)
        if swapped and label in {"1", "2"}:
            label = "2" if label == "1" else "1"
        return label
    if family == "total":
        return _over_under_side(outcome)
    return None


def _event_group_key(market):
...
def _book_event_groups_v8(markets, bookmaker):
    groups = {}
    for market in markets:
...
def _representative_market_v8(group):
    # Prefer a canonical main-result market, then any market with good event data.
    ranked = []
    for market in group:
        desc = _market_descriptor_v8(market)
        priority = {"1x2": 0, "moneyline": 1, "total": 2, "spread": 3}.get(
            desc.get("family") if desc else None, 9
        )
...
def find_event_matches_v8(markets, threshold=0.82):
    stake_groups = _book_event_groups_v8(markets, "Stake")
    shuffle_groups = _book_event_groups_v8(markets, "Shuffle")
    shuffle_reps = [(g, _representative_market_v8(g)) for g in shuffle_groups]
    candidates = []

    for sg in stake_groups:
        sm = _representative_market_v8(sg)
        if not sm:
            continue
        best = None
...
def _descriptor_map_v8(group, swapped=False):
    result = {}
    for market in group:
        desc = _market_descriptor_v8(market, swapped=swapped)
        if not desc:
            continue
...
def _expected_labels_v8(family):
    if family == "1x2":
        return ("1", "X", "2")
    if family in {"moneyline", "spread"}:
        return ("1", "2")
...
def _best_prices_v8(stake_markets, shuffle_markets, descriptor, swapped):
    best = {}
    for bookmaker, group, is_swapped in (
        ...
        for market in group:
            # Recompute to make sure this concrete market belongs to the same key.
            desc = _market_descriptor_v8(market, swapped=is_swapped)
            if not desc or desc.get("key") != descriptor.get("key"):
                continue
            for outcome in market.get("outcomes", []) or []:
                label = _canonical_selection_v8(market, outcome, desc, swapped=is_swapped)
                if label not in _expected_labels_v8(descriptor.get("family")):
                    continue
                try:
                    odds = float(outcome.get("odds"))
                except (TypeError, ValueError):
...
def find_multimarket_arbitrage_v8(markets, bankroll, min_arb_percent):
    results = []
    seen = set()
    for pair in find_event_matches_v8(markets):
        stake_map = _descriptor_map_v8(pair["stakeMarkets"], swapped=False)
        shuffle_map = _descriptor_map_v8(pair["shuffleMarkets"], swapped=pair["swapped"])
        common_keys = sorted(set(stake_map) & set(shuffle_map))

        for key in common_keys:
            descriptor = stake_map[key]["descriptor"]
            labels = _expected_labels_v8(descriptor.get("family"))
            if not labels:
                continue
            best = _best_prices_v8(
...
def _market_family_counts_v8(markets, bookmaker):
    counts = {name: 0 for name in V8_SUPPORTED_FAMILIES}
    keys = set()
    for market in markets:
        if market.get("bookmaker") != bookmaker:
            continue
        ...
        if not desc:
            continue
        signature = (_event_group_key(market), desc["key"])
        if signature in keys:
            continue
        keys.add(signature)
        counts[desc["family"]] += 1
    counts["totalComparableMarkets"] = sum(counts.values())
    return counts
...
def shuffle_debug():
    ...
    return jsonify({
        ...
        "bodyConfigured": bool(SHUFFLE_GRAPHQL_BODY) or SHUFFLE_SCAN_MODE != "CUSTOM",
        "pagesLoaded": data.get("pagesLoaded", 0),
        "fixturesDiscovered": data.get("fixturesDiscovered", 0),
...
def fetch_both_books_parallel():
    """Fetch Stake and Shuffle concurrently so one web request does not add both scan times."""
...
def match_debug():
    try:
        ...
        matches = find_event_matches_v8(combined)
        return jsonify({
            "ok": True,
            "version": SCANNER_VERSION,
            ...
            "stakeComparable": _market_family_counts_v8(combined, "Stake"),
            "shuffleComparable": _market_family_counts_v8(combined, "Shuffle"),
            "matchedEvents": len(matches),
            "sampleMatches": [
                {
                    "stakeEvent": x["stake"].get("event"),
                    "shuffleEvent": x["shuffle"].get("event"),
                    "stakeStart": x["stake"].get("startTime"),
                    "shuffleStart": x["shuffle"].get("startTime"),
                    "score": round(x["score"], 4),
                    "swapped": x["swapped"],
                    "commonMarkets": sorted(
                        set(_descriptor_map_v8(x["stakeMarkets"]))
                        & set(_descriptor_map_v8(x["shuffleMarkets"], swapped=x["swapped"]))
                    ),
                }
                for x in matches[:20]
            ],
...
def opportunities():
    ...
    try:
        stake_data, shuffle_data, fetch_seconds = fetch_both_books_parallel()
        combined = stake_data.get("markets", []) + shuffle_data.get("markets", [])
        arbs = find_multimarket_arbitrage_v8(combined, bankroll, minimum)
    except Exception as exc:
        return jsonify({
            "ok": False,
            "version": SCANNER_VERSION,
    ...
    family_counts = {name: 0 for name in V8_SUPPORTED_FAMILIES}
    for arb in arbs:
    ...
    return jsonify({
        ...
        "supportedFamilies": list(V8_SUPPORTED_FAMILIES),
        "safetyRule": "Totals/handicaps are limited to half lines (x.5); push/quarter-line markets are excluded.",
        "stakeMarketsLoaded": len(stake_data.get("markets", [])),
        "shuffleMarketsLoaded": len(shuffle_data.get("markets", [])),
        "stakeComparable": _market_family_counts_v8(combined, "Stake"),
        "shuffleComparable": _market_family_counts_v8(combined, "Shuffle"),
