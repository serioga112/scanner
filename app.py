...
def _spin_http_session():
    s = requests.Session()
    s.headers.update({
        "Accept": "application/json, text/plain, */*",
        "Content-Type": "application/json",
        "Origin": SPIN_ORIGIN,
        "Referer": SPIN_ORIGIN.rstrip("/") + "/",
        "Cache-Control": "no-cache",
        "Pragma": "no-cache",
        "Accept-Language": "en-US,en;q=0.9",
        "User-Agent": (
...
def _spin_http_post(session, payload, sid=None):
    headers = {}
    if sid:
        # BetConstruct's documented Long Poll transport requires this header
        # on every command after request_session.
    ...
    if response.status_code != 200:
        info["bodyPreview"] = (response.text or "")[:1500]
        return None, info
...
def fetch_spinandbet_markets():
    """
    Use BetConstruct SWARM's documented Long Poll transport instead of WebSocket.

    Flow:
      1) HTTPS POST request_session
      2) read SID from response
      3) HTTPS POST get with swarm-session: <SID>
      4) parse the returned Soccer 1X2 snapshot

    No account login, cookies, Cloudflare tokens, or browser-session credentials
    are copied into the server.
    """
    sent = []
    http_trace = []
    session = _spin_http_session()

    try:
        session_msg = _request_session_message()
        sent.append({"kind": "request_session", "rid": session_msg["rid"]})

        session_response, session_http = _spin_http_post(session, session_msg)
        http_trace.append({"kind": "request_session", **session_http})

        if not isinstance(session_response, dict):
            return {
                "ok": False,
                "stage": "request_session_http",
                "markets": [],
                "sent": sent,
                "httpTrace": http_trace,
                "errors": [
                    "SpinAndBet Long Poll request_session did not return JSON."
                ],
            }
        ...
        if session_response.get("code") != 0:
            return {
                ...
                "httpTrace": http_trace,
                "errors": [
                    f'Swarm request_session failed: '
                    f'{session_response.get("msg") or session_response}'
                ],
        ...
        if not sid:
            return {
                "ok": False,
                "stage": "request_session_sid",
                "markets": [],
                "sent": sent,
                "httpTrace": http_trace,
                "errors": ["request_session succeeded but no SID was returned."],
                "sessionResponse": session_response,
            }
        ...
        snapshot_response, snapshot_http = _spin_http_post(
            session, snapshot_msg, sid=sid
        )
        http_trace.append({"kind": "soccer_1x2_snapshot", **snapshot_http})
        ...
        if not isinstance(snapshot_response, dict):
            return {
                ...
                "httpTrace": http_trace,
                "errors": [
                    "Session succeeded, but the Long Poll get did not return JSON."
                ],
                "sessionResponse": session_response,
            }
        ...
        if snapshot_response.get("code") != 0:
            return {
                ...
                "errors": [
                    f'Swarm get failed: '
                    f'{snapshot_response.get("msg") or snapshot_response}'
                ],
                "sessionResponse": session_response,
