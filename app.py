...
SCANNER_VERSION = "v14-continuous-alerts"
...
def _background_scanner_loop_v14():
    with _SCAN_STATE_LOCK:
        _SCAN_STATE["startedAt"] = _utc_now_iso_v14()
    # Give Gunicorn a few seconds to finish booting before the first heavy fetch.
    time.sleep(5)
    while True:
...
def _start_background_scanner_v14():
    ...
    _BACKGROUND_THREAD = threading.Thread(
        target=_background_scanner_loop_v14,
...
def twoway_debug():
    try:
        ...
        return jsonify({
            ...
            "stakeFixtureLimit": STAKE_FIXTURE_LIMIT,
...
def opportunities():
    ...
    return jsonify({
        ...
        "safetyRule": "Only full-match Total/Handicap half-lines (x.5) are used. Team totals, period markets, whole lines, and quarter lines are excluded.",
        "stakeFixtureLimit": STAKE_FIXTURE_LIMIT,
        "stakeDetailWorkers": STAKE_DETAIL_WORKERS,
        "stakeRawFixturesDiscovered": stake_data.get("rawFixturesDiscovered"),
        "stakeFixturesLoaded": len(stake_data.get("fixtures", [])),
...
# Start one daemon scanner per Gunicorn worker. Keep Gunicorn at its default one
# worker for duplicate-free alerts, e.g. gunicorn --timeout 90 app:app.
_start_background_scanner_v14()
